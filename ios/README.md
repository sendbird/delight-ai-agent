...
AIAgentMessenger.updateSessionInfo(
    with: .manual(
        ...
        aiAgentSessionDelegate: self
    )
)
```

> **Important:** When using Manual Session, you **must** implement an `AIAgentSessionDelegate` to handle session expiration and token refresh. See the [Handle session expiration](#handle-session-expiration) section below for implementation details.
...
> - Session expiration handling is **not required** (no AIAgentSessionDelegate needed)
                   ...
                   let sessionToken = UserDefaults.standard.string(forKey: "sessionToken") {
                    ...
                    AIAgentMessenger.updateSessionInfo(
                        with: .manual(
                            userId: userId,
                            sessionToken: sessionToken,
                            aiAgentSessionDelegate: SessionManager.shared
                        )
                    )
                }
            case .failure(let error):
...
When using `ManualSessionInfo`, session tokens can expire over time due to security policies. The SDK uses an `AIAgentSessionDelegate` to notify your app when the session needs attention, allowing you to refresh tokens or redirect users to login.
...
#### Understanding AIAgentSessionDelegate callbacks

The `AIAgentSessionDelegate` provides three important callbacks:
...
Here's how to implement an AIAgentSessionDelegate:
...
#### Setting up the AIAgentSessionDelegate

Pass your AIAgentSessionDelegate when creating the ManualSessionInfo:
