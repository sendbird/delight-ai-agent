...
// Accessibility
A11Y_OPEN_CONVERSATION: 'Open conversation with Support',
...
A11Y_CSAT_RATING_LABEL: (score, label) => `${score} out of 5 — ${label}`,
...
A11Y_FILE_PDF: (name) => `PDF file: ${name}`,
...
A11Y_CONVERSATION_ITEM: (agent, status, time, count) => `${agent}, ${status}, ${time}${count > 0 ? `, ${count} unread ${count === 1 ? 'message' : 'messages'}` : ''}`,
A11Y_CONNECT_AGENT: 'Talk to human agent',
A11Y_CLOSE_MESSENGER: 'Close messenger',
A11Y_DISMISS_NUDGE_MESSAGE: 'Dismiss message',
