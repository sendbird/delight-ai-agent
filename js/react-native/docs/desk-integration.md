...
To get the latest ticket data, call the `refresh` method on an existing `DeskTicketInterface` instance. This method returns a new `DeskTicketInterface` instance with the most recent information from the server.
...
| Method | Description |
|---|---|
| getTicket(ticketId: number) | Retrieves a Desk ticket by its ID. Returns a `Promise<DeskTicketInterface>`. |

### DeskTicketInterface

`DeskTicketInterface` represents a Sendbird Desk ticket linked to a conversation.
